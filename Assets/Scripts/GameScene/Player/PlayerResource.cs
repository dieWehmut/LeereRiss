...
public class PlayerResource : MonoBehaviour
{
    ...
    void Awake()
    {
        if (healthBar == null)
        {
            ...
            if (healthBar == null)
            {
                // Fallback: search scene for a HealthBar (useful when HUD is not a child of the player)
                healthBar = FindObjectOfType<HealthBar>(includeInactive: true);
            }
        }
        ...
        if (manaBar == null)
        {
            ...
            if (manaBar == null)
            {
                // Fallback: search scene for a ManaBar (useful when HUD is not a child of the player)
                manaBar = FindObjectOfType<ManaBar>(includeInactive: true);
            }
        }
    ...
    // 增加归一化的生命值（例如 0.02 表示增加 2%）
    public void AddHealthNormalized(float delta)
    {
        if (delta <= 0f) return;
        CurrentHealth = Mathf.Clamp01(CurrentHealth + delta);
        UpdateHealthUI();
    }
